...
| T-004 | admin | 管理员当前身份查询 | P1 | todo | unassigned | T-003 | `admin` 查询接口、DTO、中文文档、测试 | 返回当前管理员账户与角色信息 |
| T-005 | admin | 管理员操作审计列表 | P1 | todo | unassigned | T-003 | `audit_logs` 查询接口、分页 DTO、中文文档、测试 | 先做管理员视角，不做复杂过滤 |
| T-006 | admin | 校园认证手工标记历史查询 | P1 | todo | unassigned | T-003、T-005 | 审计筛选接口、中文文档、测试 | 面向后台排障与追踪 |
| T-007 | auth | 校园扫码登录 adapter 预留数据层 | P1 | todo | unassigned | T-001 | `campus_app_login_sessions` 读写层、中文文档、测试 | 不接真实校园协议 |
| T-008 | user | 用户身份绑定管理查询 | P2 | todo | unassigned | T-001 | 管理视角查询接口、中文文档、测试 | 配合后台核验使用 |
| T-009 | file_storage | 对象存储统一接口 MVP | P1 | todo | unassigned | 后端存储文档已确认 | storage interface、MinIO 实现、中文文档、测试 | 兼容未来 OSS/COS |
| T-010 | search | Elasticsearch 接入骨架 | P2 | blocked | unassigned | ES 环境部署完成 | 搜索 adapter、索引任务骨架、中文文档 | 当前缺少 ES 运行环境 |
